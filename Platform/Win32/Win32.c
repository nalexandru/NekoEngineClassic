...
Sys_CpuName(void)
{
	if (!_cpu[0]) {
		...
		if (!_cpu[0]) {
			HKEY key;
			DWORD size = 0;
			char *name = Sys_Alloc(sizeof(_cpu), 1, MH_Transient);

			if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, "HARDWARE\\DESCRIPTION\\System\\CentralProcessor\\0", 0, KEY_READ, &key)
					== ERROR_SUCCESS) {
				size = sizeof(_cpu);
				RegQueryValueExA(key, "ProcessorNameString", 0, 0, (LPBYTE)name, &size);

				RegCloseKey(key);
			}

			snprintf(_cpu, sizeof(_cpu), "%s", Rt_SkipWhitespace(name));
		}

		if (!_cpu[0]) // if the above fails, we're out of options
...
Sys_InitPlatform(void)
{
	...
	_k32 = LoadLibraryA("kernel32");
	if (!_k32)
		return false;

	Win32_user32 = LoadLibraryA("user32");
	if (!Win32_user32)
		return false;
