...
// Raw Input
static BOOL (WINAPI *_RegisterRawInputDevices)(IN PCRAWINPUTDEVICE, IN UINT, IN UINT);
static UINT (WINAPI *_GetRawInputData)(IN HRAWINPUT, IN UINT, OUT LPVOID, IN OUT PUINT, IN UINT);
static LRESULT (WINAPI *_DefRawInputProc)(IN PRAWINPUT *, IN INT, IN UINT);
...
In_SysInit(void)
{
	...
	_RegisterRawInputDevices = (BOOL (WINAPI *)(IN PCRAWINPUTDEVICE, IN UINT, IN UINT))
		GetProcAddress(Win32_user32, "RegisterRawInputDevices");
	if (_RegisterRawInputDevices) {
		RAWINPUTDEVICE rid[2];

		_GetRawInputData = (UINT (WINAPI *)(IN HRAWINPUT, IN UINT, OUT LPVOID, IN OUT PUINT, IN UINT))
			GetProcAddress(Win32_user32, "GetRawInputData");
		_DefRawInputProc = (LRESULT (WINAPI *)(IN PRAWINPUT *, IN INT, IN UINT))
			GetProcAddress(Win32_user32, "DefRawInputProc");
...
HandleInput(HWND wnd, LPARAM lParam, WPARAM wParam)
{
	...
	if (_GetRawInputData((HRAWINPUT)lParam, RID_INPUT,
	...
	if (raw->header.dwType == RIM_TYPEKEYBOARD) {
		...
		default: key_code = Win32_keymap[raw->data.keyboard.VKey]; break;
		}
	...
	_DefRawInputProc(&raw, 1, sizeof(RAWINPUTHEADER));
}
...
_deadzone(float *x, float *y, const float max, const float deadzone)
{
	...
	float mag = (float)sqrt(x_val * x_val + y_val * y_val);
