...
bool
Sys_InitThread(Thread *t, const char *name, void (*proc)(void *), void *args)
{
	HANDLE thread;
	...
	if (k32_SetThreadDescription)
		k32_SetThreadDescription(thread, Rt_MbsToWcsTransient(name));
...
Sys_AtomicAdd(volatile int32_t *i, int32_t v)
{
	...
	return InterlockedExchangeAdd((LONG *)i, v) + v;
#else
	return InterlockedAdd((LONG *)i, v);
#endif
}
...
{
	return InterlockedCompareExchange((LONG *)i, e, c);
}
