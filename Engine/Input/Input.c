		...
		if (!strncmp(line, "[VirtualAxis]", len)) {
			...
			case CFG_VA: {
				...
				*ptr++ = 0x0;
				...
				In_CreateVirtualAxis(buff, (enum Button)a, (enum Button)b);
			} break;
			case CFG_AM: {
				ptr = strchr(line, '=');
				*ptr++ = 0x0;
...
enum Axis
In_CreateVirtualAxis(const char *name, enum Button min, enum Button max)
{
	uint32_t axis = 0;
	const struct KeyAxis ka = { max, min, Rt_HashString(name) };
