...
E_InitJobSystem(void)
{
	...
	if (E_GetCVarBln("Job_ForceSerial", false)->bln || (Sys_NumCpus() == 1 && !E_GetCVarBln("Job_ForceParallel", false)->bln)) {
		_serial = true;
		Sys_LogEntry(JS_MOD, LOG_INFORMATION, "Single CPU system; jobs will be executed serially.");
		return true;
	}
...
E_DispatchJobs(uint64_t count, JobProc proc, void **args, void (*completed)(uint64_t))
{
	...
	if (_serial) {
		...
		for (i = 0; i < count; ++i) {
			proc(0, args ? args[i] : NULL);
			if (completed)
				completed(++_submittedJobs);
		}
