...
static inline bool _AddComponent(struct Scene *s, struct Entity *ent, CompTypeId type, CompHandle handle);
static inline bool _CreateComponent(struct Scene *s, struct Entity *ent, CompTypeId type, const void **args);

EntityHandle
E_CreateEntityS(struct Scene *s, const char *typeName)
{
	uint64_t hash = 0;
...
bool
E_RegisterEntityType(const char *typeName, const CompTypeId *compTypes, uint8_t typeCount)
{
