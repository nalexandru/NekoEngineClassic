...
CompHandle E_CreateComponentS(struct Scene *s, const char *typeName, EntityHandle owner, const void **args);
static inline CompHandle
E_CreateComponent(const char *typeName, EntityHandle owner, const void **args) { return E_CreateComponentS(Scn_activeScene, typeName, owner, args); }
...
CompTypeId E_ComponentTypeId(const char *type_name);
...
const struct Array *E_GetAllComponentsS(const struct Scene *s, CompTypeId type);
static inline const struct Array *E_GetAllComponents(CompTypeId type) { return E_GetAllComponentsS(Scn_activeScene, type); }

bool E_RegisterComponent(const char *name, size_t size, size_t alignment, CompInitProc init, CompTermProc release);
